...
# -- 생성되는 스크립트 안에서 설정 폴더를 다시 계산하지 않고, 이 함수가 실행되는
#    시점(= 잡 제출 시점, 로그인 노드)에 해석한 절대경로를 문자열에 박아 넣는다.
#    계산 노드가 $CCpy_HOME 같은 환경변수를 물려받지 못하면, 스크립트가 조용히
#    다른 폴더(또는 프로덕션 ~/.CCpy)를 읽게 되기 때문이다.
QUEUE_CONFIG_PLACEHOLDER = "@@CCPY_QUEUE_CONFIG_PATH@@"
...
def write_diffusivity_data(crt, specie, specie_distance, temp):
    ...
    chg_data = {"H": "+", "Li": "+", "Na": "+", "K": "+", "Rb": "+", "Cs": "+",
                ...
                "F": "-", "Cl": "-", "Br": "-", "I": "-", "O": "2-"}
    if crt >= start_num:
        os.system("analyze_aimd.py diffusivity %s%s run 1 %d %.2f -msd msd_%dK.csv >> anal.log" % (specie, chg_data.get(specie, "+"), crt, specie_distance, temp))
...
if __name__ == "__main__":
    ...
    while "loop.done" not in os.listdir("./"):
        ...
        crt_step += 1
"""
    string = string.replace(QUEUE_CONFIG_PLACEHOLDER,
                            str(ccpy_config.queue_config_path()))
    return string
