...
from CCpy.Package.Diffusion.aimd.diffusion import DiffusivityAnalyzer, ErrorAnalysisFromDiffusivityAnalyzer, \
...
def write_ASD_data(csvfile):
    """
    Created by Byeongsjun Jun to add another criteria
    """
    import pandas as pd
    import numpy as np
...
def Analyze_VASP_MD(args):
    ...
    pwd = os.getcwd()
    crt_d = pwd.split("/")[-1]
    datafilename = "Mo_%s_data.csv" % crt_d
    ...
    if da.diffusivity > 0:
        rsd = summary_info['diffusivity_relative_standard_deviation']
        diffusivity = summary_info['diffusivity']
        d_error = rsd * diffusivity
        f.write("%d,%.4f,%.12f,%.12f\n" % (args.runs_end, rsd, diffusivity, d_error))
        f.close()
        write_ASD_data(datafilename)
...
def Analyze_arrhenius(args):
    ...
    if args.poscar:
        ...
        struct = IStructure.from_file(args.poscar)
