...
# Add def get_site_partial_p_dos, def get_site_partial_d_dos
...
class CompleteDos(Dos):
    ...
    def get_site_partial_p_dos(self, site):
        """
        origin from get_site_t2g_eg_resolved_dos
        """
        px_dos = []
        py_dos = []
        pz_dos = []
            ...
            if s == site:
                for orb, pdos in atom_dos.items():
                    if orb == Orbital.px:
                        ...
                        pz_dos.append(pdos)
        return {"px": Dos(self.efermi, self.energies,
                           six.moves.reduce(add_densities, px_dos)),
                "py": Dos(self.efermi, self.energies,
                           six.moves.reduce(add_densities, py_dos)),
                "pz": Dos(self.efermi, self.energies,
                           six.moves.reduce(add_densities, pz_dos))}

    def get_site_partial_d_dos(self, site):
        """
        origin from get_site_t2g_eg_resolved_dos
        """
        dxy_dos = []
        ...
        for s, atom_dos in self.pdos.items():
            if s == site:
                for orb, pdos in atom_dos.items():
                    if orb == Orbital.dxy:
                        dxy_dos.append(pdos)
                    elif orb == Orbital.dyz:
                        dyz_dos.append(pdos)
                    elif orb == Orbital.dxz:
                        dxz_dos.append(pdos)
                    elif orb == Orbital.dx2:
                        dx2_dos.append(pdos)
                    elif orb == Orbital.dz2:
                        dz2_dos.append(pdos)
        return {"dxy": Dos(self.efermi, self.energies,
                           six.moves.reduce(add_densities, dxy_dos)),
                "dyz": Dos(self.efermi, self.energies,
                           six.moves.reduce(add_densities, dyz_dos)),
                "dxz": Dos(self.efermi, self.energies,
                           six.moves.reduce(add_densities, dxz_dos)),
                "dx2": Dos(self.efermi, self.energies,
                           six.moves.reduce(add_densities, dx2_dos)),
                "dz2": Dos(self.efermi, self.energies,
                           six.moves.reduce(add_densities, dz2_dos))}
