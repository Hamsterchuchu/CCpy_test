    ...
    def get_rotate_plot(self, xlim=None, ylim=None, elt_orders=None, color_order=None):
        ...
        for i, key in enumerate(keys):
            ...
            if self.stack:
                ...
                plt.plot(y, x, color=colors[i % ncolors],
                         label=str(key), linewidth=2)
            #if not self.zero_at_efermi:
            #    ylim = plt.ylim()
            #    plt.plot([self._doses[key]['efermi'],
            ...
            if ticks['label'][i] is not None:
                ...
                if i != 0:
                    if ticks['label'][i] == ticks['label'][i - 1]:
                        ...
                        plt.axvline(ticks['distance'][i], color='gray', lw="0.5")
                else:
...
##            plt.axhline(ef, linewidth=2, color='k')
    ...
    # -- Made function
    def get_contrib_projected_plot(self, zero_to_efermi=True, ylim=None, occupancy=None, colors=None,
                                   vbm_cbm_marker=False, smooth_tol=None, line_width=3):
        """
        Get a matplotlib object for the bandstructure plot.
        Blue lines are up spin, red lines are down
        ...
        for d in range(len(data['distances'])):
            for i in range(self._nb_bands):
                for j in range(len(data['energy'][d][str(Spin.up)][i]) - 1):
                    plt.plot([data['distances'][d][j], data['distances'][d][j + 1]],
                             [data['energy'][d][str(Spin.up)][i][j], data['energy'][d][str(Spin.up)][i][j + 1]],
                             ls='-', lw=band_linewidth, color=colors[Spin.up][i][j])
